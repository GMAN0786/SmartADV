...
def build_prompt_video() -> str:
    # 동영상 고유 정보 및 추가 맥락이 담긴 video_context.txt 동적 로드
    video_context_path = OUTPUT_DIR / "video_context.txt"
    video_context = ""
    if video_context_path.exists():
        try:
            video_context = video_context_path.read_text(encoding="utf-8").strip()
        except Exception as e:
            print(f"[Warning] video_context.txt 읽기 실패: {e}")
    ...
    prompt_lines: List[str] = [
        ...
        "   - 제공된 비디오 클립은 전후 맥락 파악을 위해 앞뒤로 1.5초씩 오버랩 패딩되어 있습니다. 각 scene 정보에 기재된 '[실제 묘사 대상 구간]' 내에서 일어나는 시각 행동만 해설 문장(text)으로 묘사해야 합니다.",
        "2. 전후 대사([전 대사], [후 대사])는 화면 밖의 행동 인과관계나 흐름을 이해하는 참고용으로만 사용하고, 출력 해설 문장에는 절대 대사를 직접 쓰지 마십시오.",
        "3. 감정 해석, 소리 묘사, 추측성 표현, 은유적 묘사는 전면 금지합니다.",
        "   반드시 시각적으로 식별 가능한 물리적 행동(예: '고개를 끄덕인다', '손을 흔든다')만 객관적으로 묘사하세요.",
        "   단, 매우 높은 확률로 해당 감정을 느낄것으로 추측되며, 그 근거가 명확한 경우에 한해 감정을 표현할 수 있습니다.",
        "4. 등장인물 및 대상(인물, 동물, 가상 캐릭터 등) 일관성 추적:",
        "   - 영상에 비인간형 인물이나 고유한 대상이 나타날 경우, 인물의 고유한 외형 특징(예: 종, 색상, 의상, 크기)을 전후 씬에서 일관되게 추적하여 동일한 캐릭터는 시종일관 같은 대명사 또는 명칭으로 표현하십시오.",
        "5. 물리적 인과관계(Cause & Effect)의 정밀 추적:",
        "   - 공중에서 물체가 날아오거나 캐릭터가 무언가에 부딪히는 액션이 발생할 경우, '하늘에서 그냥 떨어졌다'고 상황을 단절하여 묘사하지 마십시오. 앞뒤 맥락(전후 패딩 영역 포함)을 입체적으로 추적하여 다른 인물이 그 물체를 던졌거나 쏘아 올린 인과적 동작이 있었는지 원인과 결과를 유기적으로 결합하여 설명해야 합니다.",
        "   - 어떤 인물이 도구(예: 끈, 나뭇가지, 밧줄, 도구 등)를 설치, 결속 또는 조립하고 있는 행동이 관찰되면, 일회성의 단순 동작이 아니라 특정 목적(예: 함정 설계, 장치 준비, 물건 제작 등)을 수행하기 위한 목적 지향적 행동임을 흐름상으로 바르게 추적하여 그 목적성과 의도가 명확히 전달되도록 해설하십시오.",
        f"6. TTS 발화 속도는 초당 약 {TTS_SYLLABLES_PER_SECOND}음절입니다.",
        "   각 scene 정보에 제공된 '최대음절' 수 제한을 엄격하게 지켜 분량을 작성해야 합니다.",
        "7. 도출된 해설을 기반으로 영상 클립의 내용이 의미왜곡 없이 정확하게 전달이 가능해야합니다.",
        "8. 한 동영상 클립 내에서 가장 핵심적이고 의미 있는 장면 변화나 동작을 위주로 설명합니다.",
        "9. 동영상 클립 내에서 설명하려는 핵심 행동이나 장면 변화가 일어나는 시점(클립 시작점 0.0초 기준 상대 시간 초)을 소수점 첫째 자리까지 정확하게 측정(추론)하세요.",
        ...
        "10. 출력은 반드시 CSV만 반환합니다. 코드블록, 설명문, 마크다운을 절대 추가하지 마십시오.",
        "",
        "[출력 CSV 스키마]",
        "silence_id,scene_id,window_start,window_end,text",
        "- silence_id: 해당 무음구간 번호 (숫자만)",
    ...
    if video_context:
        prompt_lines.insert(4, "[해당 동영상 전용 추가 맥락 및 등장인물 가이드 (Video Specific Guide)]")
        prompt_lines.insert(5, video_context)
    ...
    if mode == "VIDEO":
        ...
        for silence_id in sorted(silences):
            ...
            for scene in silence.scenes:
                ...
                # 비디오 클립 내 실제 묘사 구간 오프셋 계산 (전후 맥락 1.5초 오버랩 패딩 반영)
                clip_start_offset = min(1.5, scene.window_start_abs)
                clip_end_offset = clip_start_offset + scene.window_duration
                ...
                metadata_header = (
                    ...
                    f"[실제 묘사 대상 구간]: 제공된 동영상 클립 내 {clip_start_offset:.1f}초 지점부터 {clip_end_offset:.1f}초 지점까지\n"
                    f"[주의사항]: 클립 앞뒤 영역은 상황 추론용 맥락일 뿐이며, 해설 텍스트(text)는 반드시 이 실제 묘사 대상 구간 내 행동만 서술해야 합니다.\n"
                    f"----------------------\n"
                )
...
def align_and_correct_csv(csv_text: str, batch_silences: Dict[int, SilenceInfo]) -> str:
    ...
    for i, exp in enumerate(expected_scenes):
        ...
        if matched_text and LLM_MODE == "VIDEO":
            # [+1.5], [+2], [+2.3], +1.5, **[+1.5]** 등 극도로 유연하게 매칭 (따옴표 및 앞뒤 구분 기호 등 추가 고려)
            match = re.match(
                r'^\s*["\'“\'`]?\s*(?:\*\*|)?(?:\[\+?\s*([0-9]+(?:\.[0-9]+)?)(?:s|sec|초)?\](?:s|sec|초)?|\+\s*([0-9]+(?:\.[0-9]+)?)(?:s|sec|초)?)(?:\*\*|)?\s*[:\-~>]*\s*(.*)',
                matched_text
            )
            if match:
