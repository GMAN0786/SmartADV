...
def build_prompt_video() -> str:
    prompt_lines: List[str] = [
        ...
        "2. 전후 대사([전 대사], [후 대사])는 화면 밖의 행동 인과관계나 흐름을 이해하는 참고용으로만 사용하고, 출력 해설 문장에는 절대 대사를 직접 쓰지 마십시오.",
        "3. 터무니없는 감정 비약이나 소리 묘사는 지양하되, 인물의 행동 변화나 상황 전개는 전후 대사의 맥락과 영상 전체의 흐름을 바탕으로 합리적으로 추론하여 묘사할 수 있습니다.",
        f"4. TTS 발화 속도는 초당 약 {TTS_SYLLABLES_PER_SECOND}음절입니다.",
        "   각 scene 정보에 제공된 '최대음절' 수 제한을 엄격하게 지켜 분량을 작성해야 합니다.",
        "5. 제공된 동영상 클립에 대하여, 클립 내의 시각적 요소를 중심으로 하되 전체 줄거리와 대사 맥락을 유기적으로 결합해 상황 전환을 유기적이고 설득력 있게 추론하여 기술하십시오.",
        "단, 배경 설명에 사용된 근거는 반드시 해당 클립내에 찾을 수 있는 요소로만 구성되어야 합니다.",
        "또한, 도출된 해설을 기반으로 영상 클립의 내용이 의미왜곡 없이 정확하게 전달이 가능해야합니다.",
        "6. 한 동영상 클립 내에서 가장 핵심적이고 의미 있는 장면 변화나 동작을 위주로 설명합니다.",
        "7. 동영상 클립 내에서 설명하려는 핵심 행동이나 장면 변화가 일어나는 시점(클립 시작점 0.0초 기준 상대 시간 초)을 소수점 첫째 자리까지 정확하게 측정(추론)하세요.",
        ...
        "9. 사건의 인과관계에서 인과관계가 유지되도록 주의합니다.",
        "",
        "[출력 CSV 스키마]",
        "silence_id,scene_id,window_start,window_end,text",
        "- silence_id: 해당 무음구간 번호 (숫자만)",
