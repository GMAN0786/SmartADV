...
def build_prompt_video() -> str:
    prompt_lines: List[str] = [
        ...
        "   - 물체가 날아오거나 충돌하는 장면이 있을 때, 원인 없이 '갑자기 나타났다'거나 '하늘에서 떨어졌다'고 지어내지 마십시오. 반드시 앞뒤 맥락(패딩 영역 포함)을 관찰하여 누가, 무엇을 했기 때문에 그 결과가 발생했는지 인과 흐름을 명확히 연결하십시오.",
        "   - 등장인물이 도구나 재료를 다루며 복잡한 작업을 수행하는 장면은 단순 나열이 아니라, 해당 행동이 어떤 목적(준비, 설치, 제작 등)을 위한 것인지 전후 맥락을 통해 파악하고, 행동의 의도가 드러나도록 묘사하십시오.",
