...
def build_prompt_video() -> str:
    prompt_lines: List[str] = [
        ...
        "해설 오디오는 각 scene의 window_start(첫 장면전환 시각) 직후부터 재생됩니다.",
        "",
        "[주요 캐릭터 및 갈등 구조 인지 가이드 (Grounding Guide)]",
        "현재 분석 대상 영상에 아래 캐릭터들이 나타나는 경우, 그 고유의 외형 및 역할 관계를 바탕으로 인과관계를 바르게 매핑하여 해설을 작성하십시오:",
        "- 거대 토끼 (Big Buck Bunny): 크고 통통하며 털이 회색인 거대한 토끼. 평화롭고 온순한 성격이지만, 다람쥐 일당의 짓궂은 괴롭힘을 겪은 뒤 그들을 응징하기 위해 나뭇가지, 덩굴, 돌멩이, 밧줄 등을 나무에 묶고 매달아 매우 정교한 물리적 함정들을 설계하고 준비합니다.",
        "- 다람쥐 일당: 영악하고 심술궂은 다람쥐(Frank - 검은 줄무늬), 날다람쥐(Rinky - 하늘을 날아다님), 설치류/친칠라(Gamera - 뚱뚱한 설치류). 토끼를 골탕 먹이기 위해 사과나 나무열매 등을 수시로 던지고 공격합니다.",
        "",
        "[핵심 규칙]",
        ...
        "4. 물리적 인과관계(Cause & Effect)를 엄격히 추적하여 묘사하십시오:",
        "   - 공중에서 무언가 날아와 캐릭터를 맞추는 액션이 나타날 경우, '하늘에서 떨어졌다'고 막연히 지어내지 말고, 앞뒤 맥락을 보고 다람쥐 일당 등의 다른 캐릭터가 물건(나무열매, 사과 등)을 던져 맞춘 유기적인 힘의 인과관계를 명확히 설명하십시오.",
        "   - 토끼가 덩굴, 나뭇가지, 돌멩이 등을 가지고 설치하거나 묶고 있는 복잡한 행동은 단순한 딴청이나 딴소리가 아니라 '다람쥐 일당을 응징할 정교한 물리적 함정을 정성스럽게 설계하고 준비하는 과정'임을 올바르게 인지하고, 행동의 목적성이 명확히 드러나도록 묘사하십시오 (예: '토끼가 함정을 만들기 위해 덩굴을 나무통에 동여멥니다').",
        f"5. TTS 발화 속도는 초당 약 {TTS_SYLLABLES_PER_SECOND}음절입니다.",
        "   각 scene 정보에 제공된 '최대음절' 수 제한을 엄격하게 지켜 분량을 작성해야 합니다.",
        "6. 도출된 해설을 기반으로 영상 클립의 내용이 의미왜곡 없이 정확하게 전달이 가능해야합니다.",
        "7. 한 동영상 클립 내에서 가장 핵심적이고 의미 있는 장면 변화나 동작을 위주로 설명합니다.",
        "8. 동영상 클립 내에서 설명하려는 핵심 행동이나 장면 변화가 일어나는 시점(클립 시작점 0.0초 기준 상대 시간 초)을 소수점 첫째 자리까지 정확하게 측정(추론)하세요.",
        ...
        "9. 출력은 반드시 CSV만 반환합니다. 코드블록, 설명문, 마크다운을 절대 추가하지 마십시오.",
        "",
        "[출력 CSV 스키마]",
        "silence_id,scene_id,window_start,window_end,text",
        "- silence_id: 해당 무음구간 번호 (숫자만)",
